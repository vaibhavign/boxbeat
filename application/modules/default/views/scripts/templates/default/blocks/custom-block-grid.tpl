...
<h2>
  <div class="blocktitle">{$custom_block_title}</div>
</h2>
<div class="blockcontent">
  <ol class="grid">
    {foreach from=$custom_block_grid_array item=product}
    <li>
      <h4>
        <div class="gridtitle"><a href="{$product.url}">{$product.title|@stripslashes}</a></div>
      </h4>
      <p class="image"> <a href="{$product.url}"><img src="{$product.image}" alt="{$product.image_title|@stripslashes}" title="{$product.image_title|@stripslashes}"/></a> </p>
      <div class="container">
