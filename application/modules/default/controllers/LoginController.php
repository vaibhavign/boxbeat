    ...
    {   //echo '<pre>';
        //print_r($_SESSION);exit;
         ...
         if(isset($_SESSION['USER']['userId']) && $checkuserverification==1){
            $this->_redirect(HTTP_SERVER.'/home');
        }
     }
            ...
            $this->_redirect(HTTP_SECURE.'/login/accountrestricted/passcode/'.$usrvcode);
                ...
                switch($tabValue){ // add more cases for different redirection , snapshot implementation
                    ...
                    case 2 :
                                ...
                                break;
                    case 3 :
                                $this->_redirect(HTTP_SECURE.'/login/customerverification/sapi'.$sapi);
                                break;   
                    default:
                                if($_SESSION['mypage']==''){
							//rint_r($_REQUEST);
								//echo HTTP_SERVER.'/myaccount';
							//	exit;
                    			//echo 'me';
								//echo HTTP_SERVER.'/phphtmledit';
...
//          $mail->setBodyHtml($bodyText)
	...
	$returnusrnameArray = $mapper->getusername($uservcodes);
        ...
        else if($pageaction=='notmyaccounts' && $valuseremail==0)
       {
                      ...
                      $mail->setBodyHtml($bodyText);
                      //$mail->send();
                      $mapper->checknotmyaccount($_POST['vcode'],$_POST['changeduseremail'],$getuserid);
                      $this->_redirect(HTTP_SERVER.'/myaccount');
            }
