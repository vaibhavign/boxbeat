 ...
 function getuserrole($mymail)
    ...
    function getUnreadmessage()
    {
      ...
      //  echo "select count(id) as total,message_type from chat where (user1=".$userName->userId." || user2=".$userName->userId.") and and readby!=-1 and readby!=".$userName->userId." group by message_type  ";exit;
        $sql=$db->query("select count(id) as total,message_type from chat where (user1=".$userName->userId." || user2=".$userName->userId.") and  readby!=-1 and readby!=".$userName->userId." group by message_type");
        ...
        $allChat = $db->query("SELECT * from chat  as c where (c.user1 =" . $userName->userId . " || c.user2 =" . $userName->userId . ") and c.readby!=-1 and c.readby!=" . $userName->userId . " and c.message_type IN ('0','1')");
        $results = $allChat->fetchAll();
        ...
        $unread['alert']=$alert;
        $unread['total']=$j+$dispute+$request+$alert;
        return  $unread;
    }
