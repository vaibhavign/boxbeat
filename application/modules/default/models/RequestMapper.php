...
class Default_Model_RequestMapper extends DML {
    ...
    public function requestForCancellationAccepted($requestid, $addressbook_id, $orderItemId) {
        ...
        if ($getrecords[0]['request_status'] == 0) {
            ...
            $this->updateRecord('chat', array('request_status' => '1'), array('id' => $getrecords[0]['chat_id']));
		...
		//Email Notication - notify customer after store owner accepts a cancellation request
            ...
            $orderData['customer_detail_page_link'] = 'http://goo2ostore.com/request/#requestdetail/rqid/' . encryptLink($requestid);
            $orderData['request_data'] = $this->select('*')
                                            ->from('order_request')
            ...
            $this->notification->triggerFire(46, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
            }
    ...
    public function requestForCancellationRejected($requestid) {
        ...
        $this->updateRecord('chat', array('request_status' => '2'), array('id' => $rs1['chat_id']));
		...
		//Email Notication - notify customer after store owner rejects a cancellation request
            if ($rs1['request_item_id'] != '') {
                $orderData = $this->order_model->getEverythingAboutAnOrder($rs1['request_item_id']);
                $orderData['customer_detail_page_link'] = 'http://goo2ostore.com/request/#requestdetail/rqid/' . encryptLink($requestid);
                $orderData['request_data'] = $this->select('*')
                                            ->from('order_request')
                                            ->where(array('request_id'=>$requestid))
            ...
            $this->notification->triggerFire(47, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
            }
    }
    ...
    public function shippingAddressChangeData($request_id,$userId) {
		...
		$sellersql = $db->query("select * from user where id='" . $getRequest[0]['request_buyer_id'] . "'");
        $getname = $sellersql->fetchAll();
        $buyer_name = $getname[0]['user_full_name'];
        $getRequest[0]['buyer_name'] = $buyer_name;
        ...
        $city = $db->query("select * from city where id='" . $getRequest[0]['city'] . "'");
        $getcity = $city->fetchAll();
        $city_name = $getcity[0]['cityname'];
        $getRequest[0]['cityname'] = $city_name;
		
		// for read unread
		$chatQuery = $db->query("select readby from chat where i_id='".$request_id."'");
		$chatRead = $chatQuery->fetchAll();
		$chatStatus = $chatRead[0]['readby'];
        if ($chatStatus == 0) { //page viewed by none of them
            $update = $db->query("Update chat set readby='" . $userId . "' where i_id='" . $request_id . "'");
        } elseif ($chatStatus == -1) {  //page viewed by both seller and buyer
            $update = $db->query("Update chat set readby='" . $userId . "' where i_id='" . $request_id . "'");
        } else {          //page viewed by one and that user's id is there
            if ($userId == $chatStatus) {
                //Do Nothing
            } elseif ($userId != $chatStatus) {
                $update = $db->query("Update chat set readby='-1' where i_id='" . $request_id . "'");
            }
        }
    ...
    public function changeInShippingAddressAccepted($requestid, $addressbook_id, $orderItemId) {
        ...
        if ($getrecords[0]['request_status'] == 0) {
            ...
            $cityname = $db->query("Select cityname from city where id = '" . $cityid . "'");
            ...
            if (empty($fetchRecords)) { //Insert in order address if empty
                ...
                $this->updateRecord('order_item', array('order_address_id' => $updatedata['order_address_id']), array('order_item_id' => $orderItemId));
            }//End if 
            else {
                $totalOrderItem = $this->getTotalOrderItem($orderItemId);
            ...
            if ($orderItemId != '') {
                ...
                $orderData['new_address'] = $this->buyerMapper->getAddressInString($addressbook_id);

                //from Goo2o to buyer
                $this->notification->triggerFire(55, array('to_id' => $orderData[0]['buyer_id'], 'orderData' => $orderData));
                //from store owner to buyer
                $this->notification->triggerFire(56, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
    ...
    public function changeInShippingAddressRejected($requestid) {
		...
		//Email Notication - Notify customer when store owner rejects an address change request
            if ($rs1['request_item_id'] != '') {
                $orderData = $this->order_model->getEverythingAboutAnOrder($rs1['request_item_id']);
            $orderData['customer_detail_page_link'] = 'http://goo2ostore.com/request/#requestdetail/rqid/' . encryptLink($requestid);
                $cityname = $this->order_model->getCityName($orderData[0]['city']);
                $statename = $this->order_model->getStateName($orderData[0]['state']);
                $orderData['old_address'] = $orderData['address'] . ',' . $cityname . ',' . $statename . ',' . $orderData['pincode'];
                $orderData['new_address'] = $this->buyerMapper->getAddressInString($orderData['request_address_book_id']);
                //from Goo2o to buyer
                $this->notification->triggerFire(53, array('to_id' => $orderData[0]['buyer_id'], 'orderData' => $orderData));
                //from seller to buyer
            $this->notification->triggerFire(54, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
            }
    }

    public function getReturnDetailFromReturnId($requestId, $userId) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $result = $this->select('*')
                ->from('order_request as orq')
                ->join('order_item as oi', 'orq.request_item_id=oi.order_item_id', 'inner')
                ->join('order_returns as orn', 'orn.return_id= oi.return_id', 'inner')
                ->join('orders as o', 'o.order_id=orn.order_id', 'inner')
                ->where(array('orq.request_id' => $requestId))
                ->get()
                ->rowArray();		
				
        $statusId = $result['order_item_status'];
		...
		//echo "select readby from chat where i_id='".$requestId."'".'fdsf'.$userId;
				// for read unread
        $chatQuery = $db->query("select readby from chat where i_id='" . $requestId . "'");
		$chatRead = $chatQuery->fetchAll();
		$chatStatus = $chatRead[0]['readby'];
        if ($chatStatus == 0) { //page viewed by none of them
            $update = $db->query("Update chat set readby='" . $userId . "' where i_id='" . $requestId . "'");
        } elseif ($chatStatus == -1) {  //page viewed by both seller and buyer
            $update = $db->query("Update chat set readby='" . $userId . "' where i_id='" . $requestId . "'");
        } else {          //page viewed by one and that user's id is there
            if ($userId == $chatStatus) {
                ...
                $update = $db->query("Update chat set readby='-1' where i_id='" . $requestId . "'");
            }
        }
    ...
    public function respondToReturnAccepted($data) {
        ...
        $rs1 = $this->select('*')
                ...
                ->get()
                ->rowArray();
        $this->updateRecord('chat', array('request_status' => '1'), array('id' => $rs1['chat_id']));
        ...
        if (!empty($feedback)) {
            ...
            $result = $this->select('order_prev_status')
                    ...
                    ->get()
                    ->rowArray();
            $updateOrderItem = $db->query("UPDATE order_item SET order_item_status='" . ORDER_STATUS_DELIVERED . "',order_sub_status_id='144',buyer_substatus='145' where order_item_id='" . $data['orderItemId'] . "'");
            ...
            $updateOrderItem = $db->query("UPDATE order_item SET order_item_status='" . ORDER_STATUS_DELIVERED . "',order_sub_status_id='148',buyer_substatus='149' where order_item_id='" . $data['orderItemId'] . "'");

        }//End else
        $this->updateRecord('order_shipment as os', array('order_shipment_status' => ORDER_SHIPMENT_DELIVERED, 'buyer_shipment_substatus' => BUYER_ORDER_SHIPMENT_SUBSTATUS_DELIVERED, 'seller_shipment_substatus' => SELLER_ORDER_SHIPMENT_SUBSTATUS_DELIVERED), array('order_item_id' => $data['orderItemId'], 'shipment_type' => 'ORDERS'));
		 ...
		 //Email Notication - notify customer after store owner accepts a return request
            if ($data['orderItemId'] != '') {
                ...
                $returndata = $this->buyerMapper->getReturnDataFromRequestID($data['requestId']);
                $orderData['returnData'] = $returndata;
                $orderData['customer_request_detail_page'] = 'http://goo2ostore.com/request/#requestdetail/rqid/' . encryptLink($data['requestId']);
                $orderData['returnIdLabel'] = $this->order_model->getReturnIdLabel($returndata['return_id']); 
                $this->notification->triggerFire(43, array('to_id' => $orderData[0]['buyer_id'], 'orderData' => $orderData));
				
            $this->notification->triggerFire(86, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
            }
    ...
    public function respondToReturnRejected($data) {
		...
		//Email Notication - notify customer after store owner rejects a return request
            if ($data['orderItemId'] != '') {
                $orderData = $this->order_model->getEverythingAboutAnOrder($data['orderItemId']);
            $return_id = $this->select('*')
                        ->from('order_item')
                        ->where(array('order_item_id'=>$data['orderItemId']))
                        ->get()
                        ->rowArray();
            $orderData['customer_request_detail_page'] = 'http://goo2ostore.com/request/#requestdetail/rqid/' . encryptLink($data['requestId']);
            $orderData['returnIdLabel'] = $this->order_model->getReturnIDLabel($return_id['return_id']); 

                $this->notification->triggerFire(42, array('to_id' => $orderData[0]['buyer_id'], 'orderData' => $orderData));
				
            $this->notification->triggerFire(85, array('to_id' => $orderData[0]['buyer_id'], 'from_id' => $orderData[0]['seller_id'], 'orderData' => $orderData), 1);
            }
    }
    
    public function getOrderItemIdFromRequestId($requestid) {
        return $this->select('request_item_id')
                ->from('order_request')
                        ->where(array('request_id' => $requestid))
                ->get()
                ->rowArray();
    ...
    public function getReturnDataFromRequestID($requestid){
        return $this->select('*')
                ->from('order_request as orq')
                ->join('order_returns as ore','orq.request_id=ore.order_return_id','inner')
                ->where(array('orq.request_id'=>$requestid))
                ->get()
                ->rowArray();
}
