...
class Secure_Model_MerchantenrollmentMapper{
	...
	public function checkUserMall(){
		$check_mall_detail = "SELECT count(*) AS mall_record , id from mall_detail WHERE apikey = '".$this->_user_api_key."' GROUP BY id";
		$total_record = $this->db->query($check_mall_detail)->fetchAll();
		$this->_mall_count_value = $total_record[0]['mall_record'];
	}
	public function userDetails(){
		$user_detail = "SELECT * FROM mall_detail WHERE apikey = '".$this->_user_api_key."'";
    ...
    public function checkMallActivate() {
		$check_mall_detail = "SELECT count(*) AS mall_record from mall_detail WHERE apikey = '".$this->_user_api_key."'";
		$total_record = $this->db->query($check_mall_detail)->fetchAll();
		if(count($total_record[0]['mall_record']) > 0){
			$check_mall_activation_query = "SELECT active FROM mall_detail WHERE active = '1' AND apikey = '".$this->_user_api_key."' GROUP BY active";
	...
	public function updateMallUrlStatus(){
		$update_status_query = "UPDATE mall_detail set active = '1' WHERE apikey = '".$this->_user_api_key."'";
