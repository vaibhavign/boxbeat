...
class Secure_MerchantenrollmentController extends Zend_Controller_Action{
    ...
    public function businessDetailsAction(){
		...
		if($_SESSION['original_login']['user'][0]['store_owner_type']=='1'){
			$this->view->business_name = stripslashes($_SESSION['USER']['stores'][0]['business_name']);
			$this->view->business_type = $_SESSION['USER']['stores'][0]['business_type'];
			$this->view->business_address = $_SESSION['USER']['stores'][0]['business_address'];
			...
			$this->view->pincode = $_SESSION['USER']['stores'][0]['pincode'];
			$this->view->store_name = $_SESSION['USER']['stores'][0]['title'];
			preg_match('@^(?:http://www.)?([^.*]+)@i',$_SESSION['USER']['stores'][0]['mallurl'], $mallurl);
    ...
    public function congratulationsAction(){
		...
		if($users_details['payment_mode'] == 3 || $users_details['transaction_id']!= ''){
			...
			if($_SESSION['ACTIVATION_CODE']['activation_code']!=''){
				...
				if($get_count_id < 1){
					$mailFlag = 1;
					$insert_data_array['user_id']=$_SESSION['original_login']['userId'];
					...
					$merchant_mapper->getAndUpdateDemoMallDetail($this->_user_api_key,$insert_data_array,$_SESSION['original_login']['user'][0]['user_email_address']);
					...
					//	$merchant_mapper->updateSameUserWithNewApiKey($get_last_id,$username_array);
						$insert_data_array['parent_store']=$_SESSION['USER']['userId'];//$insert_data_array['user_id'];
						$merchant_mapper->getAndUpdateDemoMallDetail($this->_user_api_key,$insert_data_array,$_SESSION['original_login']['user'][0]['user_email_address']);
						$get_api_key = $merchant_mapper->__get('_demoapikey');
						$this->_store_api_key = $get_api_key;
						$get_last_id  = $_SESSION['original_login']['userId'];
						...
						$merchant_mapper->updateDemoMallDetails($insert_data_array,$_SESSION['USER']['stores'][0]['id']);
			...
			$this->_redirect(HTTP_SERVER.'/merchantenrollment/business-details');// $this->_redirect(HTTP_SECURE.'/merchantenrollment/review-details');
		}
    }
