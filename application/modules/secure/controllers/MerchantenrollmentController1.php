...
class Secure_MerchantenrollmentController extends Zend_Controller_Action{
    ...
    public function businessDetailsAction(){
		...
		if($_POST['action'] == 'process' || $_SESSION['CHECKFLAG']['flag'] == 1){ // condition for save the data and check session data on back button
			...
			$insert_data_array = array('business_name'=>trim($this->view->business_name),'business_type'=>$this->view->business_type,'business_address'=>trim($this->view->business_address),'city'=>$city_id,'state'=>$state_id,'pincode'=>trim($this->view->pincode),'title'=>trim($this->view->store_name),'mallurl'=>$mall_url,'user_id'=>$user_id);
			$_SESSION['ENROLLMENT'] = $insert_data_array;
			if($_POST['continue_business_detail']=='continue_to_payment'){
    ...
    public function paymentDetailsAction(){
		...
		if($_POST['continue_payment_btn'] == 'continue' || $_SESSION['CHECKFLAG']['flag'] == 0){
			...
			elseif($_POST['checkbox_input']=='checkbox_on' || $checkbox_input == 2){
		...
		if($users_details['payment_mode'] == 3 || $users_details['transaction_id']!= ''){
			...
			if($_SESSION['ACTIVATION_CODE']['activation_code']!=''){
				...
				if($get_count_id < 1){
					...
					$merchant_mapper->addSameUserWithNewApiKey($user_seesion_value);
					$get_last_id = $merchant_mapper->__get('_last_user_inserted_id');
					$get_new_api_key = $merchant_mapper->__get('_new_api_key');
					$username_array = array('name'=> $user_session_val['user_email_address'].'-'.$get_last_id,'password' => md5($user_session_val['username'].'-'.$get_last_id),'username' => $user_session_val['username'].'-'.$get_last_id,'apikey'=>$get_new_api_key);
					...
					$merchant_mapper->addBusinessDetails($insert_data_array,$get_new_api_key);//data insert into mall_detail table
					...
					//Insert In User_role ANd user_permission TAble
					$merchant_mapper->mallOwnerRolePermission($get_new_api_key,$_SESSION['original_login']['user'][0]['user_email_address']);
				}
			...
			$mapper=new Admin_Model_IndexMapper();
		    	$mapper->changeUserprofile('',$get_last_id);
