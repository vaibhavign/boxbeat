...
class Secure_LoginController extends Zend_Controller_Action {
  ...
  if($_SESSION['USER']['userId']!='' && $_GET['tab']==1)
  {
  	 $this->_redirect('https://goo2o.com/o2ocheckout/selectshippingaddress?tab='.base64_encode(session_id()));
  }
  ...
  if($_REQUEST['request']!='' && $_REQUEST['self']!=1)
        ...
        if($Inputs['action']=='index' && $Inputs['controller']=='login')
        {
    ...
    public function indexAction() {
        ...
        if ($pos = strpos($_POST['username'], '-')) {
            ...
            $class = new General();
            $feed = HTTP_SECURE . "/signin/json?username=" . trim($_POST['username']). "&password=" . trim($_POST['password']);
            $urlData = curl($feed);
		...
		$activeme = $vcodes[0]['active'];
                $this->view->restrictme=0;
                if($activeme==0){
                    $this->view->restrictme=1;
                    return false;
                }
                if ($checkcanceluser > 0) {
                ...
                setcookie("asess", $apiKey, time() + 3600, "/", ".o2o.com");
                ...
                switch ($tabValue) { // add more cases for different redirection , snapshot implementation
                    case 1 :
						if($usersid=='34' || $usersid='865')
						{

						//echo 'https://www.secure.goo2o.com/o2ocheckout/selectshippingaddress?tab='.base64_encode(session_id());exit;
						  $this->_redirect('https://goo2o.com/o2ocheckout/selectshippingaddress?tab='.base64_encode(session_id()));
						}
						else
						{
					...
					if ($_SESSION['mypage'] == ''){
						{
							...
							//$this->_redirect(HTTPS_SERVER . '/phphtmledit');
						} else if($_SESSION['mypage'] == 'http://secure.goo2ostore.com/demo/create-my-free-demo-store') {
...
// end function indexAction
    ...
    public function forgetpasswordinstructionAction() {
        ...
        $this->view->useremailaddress = $newuseremail;
    }// end of forgetpasswordinstruction action
    ...
    }// end of forgetpasswordresendinstruction action
...
// end function index action
    ...
    public function resetpasswordinstructionAction() {
        ...
        $this->view->headScript()->appendFile('/jscript/common/o2ojslib.js', 'text/javascript');
        $this->view->headLink()->appendStylesheet('/css/secure/stylesheet_header.css');
    }// end of resetpasswordinstruction action
    ...
    public function notmyaccountAction() {
        ...
        if ($pageaction == 'notmyaccounts' && $newuseremail == $email) {
            ...
            $mapper = new Secure_Model_LoginMapper();
            $getuservcode =$class->makeUrl($newuserid);
            $mapper->checknotmyaccount($getuservcode, $_POST['changeduseremail'], $getuserid,$newuseremail);
    ...
    }// end of youremailremoved action
    ...
    public function notyouraccountAction() {
        ...
        if($newuserid==''){
                ...
                $newuseremail = $returnusrnameArray[0]['user_email'];
                $this->view->useremailaddress = $newuseremail;
                $returnususername = $mapper->accessusername($newuserid);
                $this->view->useremailname = $returnususername;
        }
        else
        {
                $this->view->usernameid = $newuserid;
                $newuseremail = $returnusrnameArray[0]['user_email_address'];
                $this->view->useremailaddress = $newuseremail;
                $this->view->useremailname = $returnusrnameArray[0]['username'];
        }
   ...
   public function accountrestrictedAction() {
        ...
        if ($pageaction == 'accountcheckprocess' && $newuseremail == $email) {
            ...
            $mapper = new Secure_Model_LoginMapper();
            $getnewuservcode =$class->makeUrl($newuserid);
            $vcode = HTTPS_SECURE . '/registration/mailconfirmation/passcode/' . $getnewuservcode;
            $notmyaccountlink = HTTPS_SECURE . '/login/notyouraccount/passcode/' . $getnewuservcode;
            $tData = array( 'account_verification_link'=>$vcode,
                                                       'not_my_account_link'=>$notmyaccountlink,
                                                       'to_id'=>$newuserid,
                                                       'to_mail'=>$email,
                                                       'to_name'=>$fullusername);
            $this->objTrigger->triggerFire(1,$tData);
            $mapper->checknotmyaccount($getnewuservcode, $email, $getuserid, $newuseremail);
            $this->_redirect(HTTP_SECURE . '/accountsetting/logout');
        }
        else if ($pageaction == 'accountcheckprocess' && $valuseremail > 0) {
            $this->view->emailExist = 1;
    ...
    }// end of accountrestricted action
    ...
    }// end of notyournewaccount action
    ...
    }// end of younewemailremoved action
    ...
    public function customerverificationAction() {
        ...
        $this->userName = new Zend_Session_Namespace('USER');
        $this->userApi = new Zend_Session_Namespace('original_login');
        $capi = $this->userApi->apikey;
        $Input = $this->_request->getParams();
        $sapi = base64_decode($Input['sapi']);
	if($this->userName->userId == '')
		{
                        $_SESSION['mypage']='/login/customerverification/sapi/'.$Input['sapi'];
    ...
    }// end of customer verification action
