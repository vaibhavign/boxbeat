...
class Secure_DemoController extends Zend_Controller_Action{
    ...
    public function init(){
		$_SESSION['mypage']=HTTP_SECURE . '/demo/create-my-free-demo-store';
    ...
    public function indexAction(){
		$this->_redirect(HTTP_SECURE . '/demo/create-my-free-demo-store');
	}
...
<div style="margin-top:70px;text-align:center;" align="center"><img src="http://goo2ostore.com/bar/images/goo2oloading.gif"></div><div style="color:#0083CB;font-size:22px; margin-top:50px; text-align:center;">Your page will redirect in 5 seconds. Problems with the redirect, please press "F5" or Ctrl+R .</div>';
			...
			$db = mysql_connect('localhost', 'sketchee_o3o', 'abc123456');
			mysql_select_db("sketchee_o6o", $db);
	...
	/*public function storeExistsAction(){
		...
		if($this->view->mall_detail[0]['store_owner_type']==1)
			$this->view->headTitle('Sorry! A GMS demo store already registered with this Goo2o ID - Goo2o.com');
		else
			$this->view->headTitle('Sorry! A GMS store already active with this Goo2o ID - Goo2o.com');
	...
	/*******************************Create a demo Store :07/02/2012*****************************************/
	public function createMyFreeDemoStoreAction(){
		$this->view->headTitle('Welcome: Create a free GMS demo store-Goo2o.com');
		$flag = 1;
		$merchant_mapper = new Secure_Model_MerchantenrollmentMapper($this->_user_api_key);
		$merchant_mapper->checkDemoActivationDays();
		$totaldays = $merchant_mapper->__get('_demo_setting_days');
		$this->view->days = ($totaldays)?$totaldays:'15';
		if($_SESSION['original_login']['userId']!=''){
			$merchant_mapper->checkUserMall();
			$get_count_id = $merchant_mapper->__get('_mall_count_value');
		}else{
		...
		if($get_count_id < 1){
			if($_POST['create_demo_x']!='' || $_POST['create_demo']!=''){
				$this->view->store_url = ($_POST['domain_name'])?$_POST['domain_name']:'';
				$this->view->store_name = ($_POST['store_name'])?$_POST['store_name']:'';
				$this->view->contact_no = ($_POST['contact_no'])?$_POST['contact_no']:'';
				if($_POST['domain_name']!=''){ // condition for check store url exist
					$merchant_mapper->checkStoreUrl(trim($_POST['domain_name']));
					...
					if($get_total_count > 0){
						...
						$this->view->url_msg_display = 'display:block;';
						$flag = 0;
					}//End check count condition
				}// condition end for check store url exist
				$mall_url = 'http://www.'.strtolower(trim($_POST['domain_name'])).'.mygoo2o.com';
				$insert_data_array = array('business_name'=>'','business_type'=>'','business_address'=>'','city'=>'','state'=>'','pincode'=>'','title'=>trim(addslashes($this->view->store_name)),'mallurl'=>$mall_url,'user_id'=>'','primary_industry'=>'','source_area'=>'','active'=>'1','parent_store'=>'0','goo2o_index'=>'4','store_owner_type'=>'1','create_date'=>time());
				if($flag !=0){
					$_SESSION['DEMO_STORE_DATA'] = $insert_data_array;
					$_SESSION['USER_CONTACT'] = $_POST['contact_no'];
					$_SESSION['DEMO_FLAG'] = 2;
					$_SESSION['DEMO_MAGANE_FLAG'] == 3;
					$this->_redirect(HTTP_SECURE.'/demo/store-created-successfully');
				}//End check Flag Condition
			}//End Check Post Condition
		}else{
			$merchant_mapper->userDetails();
			$mall_detail = $merchant_mapper->__get('_user_details_array');
			if($mall_detail[0]['store_owner_type'] == 1 && $mall_detail[0]['active'] == 1)
				$this->_redirect(HTTP_SECURE . '/demo/demo-currently-active');
			else if($mall_detail[0]['store_owner_type'] == 1 && $mall_detail[0]['active'] == 0)
				$this->_redirect(HTTP_SECURE . '/demo/demo-currently-inactive');
			else
				$this->_redirect(HTTP_SECURE . '/demo/regular-goo2ostore');
		}//End if condition for check user already exist or not	
	}
	
	public function storeCreatedSuccessfullyAction(){
		if($this->_user_name->userId == ''){
			$this->_redirect(HTTP_SECURE . '/login');
		}
		$_SESSION['mypage'] = '';
		$this->view->headTitle('Congratulation! Your free GMS demo store is now live-Goo2o.com');
		$merchant_mapper = new Secure_Model_MerchantenrollmentMapper($this->_user_api_key);// object for Business mapper class
		$merchant_mapper->checkDemoActivationDays();
		$totaldays = $merchant_mapper->__get('_demo_setting_days');
		$this->view->days = ($totaldays)?$totaldays:'15';
		...
		if($get_count_id < 1){
			if($_SESSION['DEMO_FLAG'] == 2){
				...
				$get_last_id  = $_SESSION['original_login']['userId'];
				//$merchant_mapper->mallOwnerRolePermission($this->_user_api_key,$_SESSION['original_login']['user'][0]['user_email_address']);
				$merchant_mapper->updateUserContactDetail($_SESSION['USER_CONTACT'],$_SESSION['DEMO_STORE_DATA']['user_id']);
				//Add value for demo user mail setting 15-02-2012
				$merchant_mapper->addUserMailSetting($demoapikey);
				//End Here	
				//Update fb table detail
				$merchant_mapper->updateFacebookDetail($demoapikey,$this->_user_api_key,$_SESSION['DEMO_STORE_DATA']['mallurl']);
				//End here
				$mapper=new Admin_Model_IndexMapper();
				$mapper->changeUserprofile($_SESSION['DEMOAPIKEY'],$get_last_id);
				$this->view->mallurl = $_SESSION['USER']['stores'][0]['mallurl'];
				$this->view->storesname = $_SESSION['USER']['stores'][0]['title'];
				$_SESSION['original_login']['user'][0]['store_owner_type'] = 1;
				$_SESSION['DEMO_STORE_DATA'] = '';
				$_SESSION['DEMO_FLAG'] = '';
				$_SESSION['DEMOAPIKEY'] = '';
				$_SESSION['USER_CONTACT'] = '';
				/*----@Trigger NO: 117 @created by : Mukesh Bisht @date : 08-02-2012 -------------*/
					  $tData = array(  'STORE_URL'=>$_SESSION['USER']['stores'][0]['mallurl'],
									   'STORE_NAME'=>ucfirst(strtolower($_SESSION['USER']['stores'][0]['title'])),
									   'STORE_OWNER_NAME'=>$_SESSION['original_login']['user'][0]['user_full_name'],
									   'to_mail'=>$_SESSION['USER']['stores'][0]['email_id'],
									   'to_id'=>$_SESSION['original_login']['userId']);
						//echo '<pre>';print_r($tData);
					  $this->objTrigger->triggerFire(14,$tData);
				/*---------------End HERE----------------------------*/
				/*----@Trigger NO: 13 @created by : Mukesh Bisht @date : 08-02-2012 -------------*/
				...
				/*---------------End HERE----------------------------*/	
				//Create Dynamic Logo And Copy To Different Server
				//$merchant_mapper->createDynamicImageAndTransfer($_SESSION['USER']['stores'][0]['title'],$demoapikey);
				//End Here
			}else{
				$this->_redirect(HTTP_SECURE.'/demo/create-my-free-demo-store');
				$_SESSION['DEMO_STORE_DATA'] = '';
	...
	public function demoCurrentlyActiveAction(){
		$_SESSION['mypage'] = '';
		if($this->_user_name->userId == ''){
			$this->_redirect(HTTP_SECURE . '/login');
		}
	...
	public function demoCurrentlyInactiveAction(){
		$_SESSION['mypage'] = '';
		if($this->_user_name->userId == ''){
			$this->_redirect(HTTP_SECURE . '/login');
		}
		...
		$merchant_mapper->userDetails();
		$this->view->mall_detail = $merchant_mapper->__get('_user_details_array');
		$this->view->headTitle('Sorry! A GMS demo store already registered	with this Goo2o ID but currently inactive - Goo2o.com');
	}
	public function regularGoo2ostoreAction(){
		$_SESSION['mypage'] = '';
		if($this->_user_name->userId == ''){
			$this->_redirect(HTTP_SECURE . '/login');
		}
