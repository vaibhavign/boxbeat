...
$errorList=array();
		...
		if($filesize > 0){	
			...
			if($filesize > $maxlimit){ 
				$errorList[] = "Maximum size of the image is 700kb";
			}
			...
			foreach($allowed_ext as $ext){
				if($ext==end($file_ext)){
					...
					@move_uploaded_file($_FILES[$fileName]['tmp_name'],'tmp/'.$newfilename);
					$save = $folder.$newfilename;
					//ho $_FILES[$fileName]['tmp_name'];exit;
					/*if(!file_exists($save)){
						list($width_orig, $height_orig) = getimagesize($save);
						if($maxH == null){
							if($width_orig < $maxW){
								$fwidth = $width_orig;
							}else{
		...
		if(!$match){
		   	$errorList[]= "File format not supported";
		}
...
?>
<?php //echo $imgUploaded; exit;
	if($imgUploaded){
		echo '<img src="'.$upload_image.'" border="0" height="100" width="100" />';
	}else{
                 echo '<div class="lh5">&nbsp;</div>';
		 echo '<img src="/images/secure/user_image/no_image.jpg" width="100" height="100px" border="0"/>';
		 foreach($errorList as $value){
                 echo '<div class="clearBoth errorMsg" id="invaliderrorMsg">'.$value.'</div>';               
		}
	}
   ?>