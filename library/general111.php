 ...
 */
 function requestActionType($type){
	switch($type){
		case 1 :$action='Need';break;
		case 2 :$action='Deel';break;
		case 3 :$action='Conversation';break;
		case 4 :$action='Comment';break;
		
		case 6 :
		case 7 :
		case 8 :
		case 9 :$action ='Edit ' ; break; 
		case 10 :$action ='Create';break;
		case 11 :$action='Report';break;
	}
	return $action;
}	
...
function shortActionType($req){
  	switch($req){
	    case 1 :
		case 10 :$action ='Created ' ;break;
		case 11 :$action ='Reported On ' ;break; 
		case 6 :
		case 7 :
		case 8 :
		case 9 :$action = 'Edited ';break;
	}
	return $action;
}	
/**
...
function editon($req){
  	switch($req){
		case 6 :$request ='Name of' ;break;
		case 7 :$request ='Long Description of' ;break;
		case 8: $request ='Short Description of';break;
		case 9: $request ='Image of';break;
	}
	return $request;
}
...
function getAgentAction($actionbyagent){
  	switch($actionbyagent){
		case 'inbox' :$action ='assigned to' ;break;
		case 'onhold' :$action ='on hold by' ;break; 
		case 'approved' :$action = 'approved by';break;
		case 'rejected' : $action = 'rejected by';break; 
	}
	return $action;
}	
...
function couponcategoryTree(){
	...
	<?php //echo "<pre>";print_r($parentArr);
...
function categoryTree(){
	...
	<?php //echo "<pre>";print_r($parentArr);exit;
		foreach($parentArr as $parent){
			if($parent['cat_id']==0){
				$noofproduct='';
			}else{
...
function draganddrop(){//echo "<pre>";print_r($_SESSION);
	...
	foreach($parentArr as $row)
	{
		...
		<li id="node_<?php echo $row[cat_id];?>">
			<a href="javascript://" rel="<?php echo $row[cat_id];?>" class="changecss"><?php echo stripslashes($row['cat_name']).'('.$this->getCount($row[cat_id]).')';?></a>
	...
	function categoryChild($id,$width=0) {
		...
		if(count($result) > 0) { $width+=20;
			...
			<?php }if(($controllerName=='addcategory')&&($actionName=='managecategory')){?><div style="float:right; display:none; *margin-top:-27px;"  class="action type_<?php echo $row[cat_id];?>" ><div class="actionName" style="cursor:pointer;">Action</div><div style="cursor:pointer" class="actionDownArrow"></div></div>
					<div class="showhidedrag action_<?php echo $row[cat_id];?>" style="display:none;">&nbsp;</div>
             <?php }?>     	
				<!--<div style="width:<?php echo $width;?>px; float:left">&nbsp;</div>--><ul><?php
				categoryChild($row['cat_id'],$width);?>
				</ul></li> 
...
//		}
	...
	function getimagedimension($imagename,$moduleName,$requiredwidth,$requiredheight){
		...
		list($width, $height) = getimagesize($imagename);                
                ...
                if($width<$requiredwidth && $height<$requiredheight){
                   $dimension[0]	= $width;
                   $dimension[1]	= $height; 
                }else if ($xRatio * $height < $requiredheight)
                { // Resize the image based on width
	...
	*/ 
       function getSubCatStr($cat_id, $str = ''){
           $db = Zend_Db_Table::getDefaultAdapter();
            $str = ($str=='')? $cat_id : $str;
            $sql = $db->select()->from(array(TABLE_ADDCATEGORY),array(cat_id))
                                      ->where("cat_id != 1 AND parent_id='".$cat_id."' AND (apikey='".$_SESSION[SESSION][ApiKey]."' or apikey='') AND (status='1')");
            $result = $db->fetchAll($sql);
            if(count($result)>0){
                foreach ($result as $key=>$val){
                    $str .= ','.$this->getSubCatStr($val['cat_id'],$str);
                }
                return $str;
            }else{
                return $cat_id;
	...
	*/ 
	...
	function categoryTreeList(){
			...
			?>
			<li id="node_<?php echo $row['cat_id'];?>" class="treeItem <?php echo $underline;?>" rel="<?php echo $width;?>"><div style="float:left; margin-left:2px; margin-right:5px; margin-top:8px; <?php echo $opacity;?>"><img src="/images/addcategory/closed.gif" class="folderImage" id="node_<?php echo $row['cat_id'];?>" /></div><span class="textHolder <?php echo $setAction;?>" id="node_<?php echo $row['cat_id']?>" rel="<?php echo $row['cat_id'];?>" style="<?php echo $opacity;?>" title="<?php if($row['cat_id']!=1){echo stripslashes($row['cat_name']).' ('.$this->getCount($row['cat_id']).')';}else{echo $row['cat_name'];}?>"><?php if($row['cat_id']!=1){echo stripslashes($row['cat_name']).' ('.$this->getCount($row['cat_id']).')';}else{echo $row['cat_name'];}?></span>
				<?php if($setAction=='place'){?><div style="float:right; display:none; margin-top:10px; *margin-top:-25px;" id="tick_<?php echo $row[cat_id];?>" class="showhidetick"><img src="/images/addcategory/tick.gif" width="12" height="9" border="0"/></div>
	...
	switch($case){
		...
		$select = $db->select()->from(array('menu'))
			...
			if($numRecords > 0){ // there is slight possibility for this condition to fail but who knows
                ...
                ';
				if($hyperlink){
					if($case == 'design' && $api_key == 'fd7cb6b8b603eeb328a9c6e4e51ff1a3'){
						$page_map = array(21=>'storedesign',22=>'pages',41=>'banners');
						$value['link'] = DESIGN_SERVER.'/'.$page_map[$value['id']]."?sessid=".Zend_Session::getId()."&key=".$api_key;
					}
				$abc .='<div class="clearBoth"><a href="'.$value['link'].'" title="'.$headingText.'"><img src="/images/'.$dir.'/'.$case.'/'.$filename.'.jpg" alt="'.$headingText.'" /></a></div>';
				} else {
				$abc .='<div class="clearBoth"><img src="/images/'.$dir.'/'.$case.'/'.$filename.'.jpg" alt="'.$headingText.'" /></div>';	
				}
                $abc .='<div class="lh20">&nbsp;</div>
                <div class="clearBoth"><a href="'.$value['link'].'" class="landingpage_link" title="'.$headingText.'">'.$headingText.'</a></div>
                <div class="lh12"></div>
...
function checkuserverification($vcodes){
        $db = Zend_Db_Table::getDefaultAdapter();
        $usrmail=$_SESSION['USER']['userDetails']['0']['user_email_address'];
        $usrjoindate=$_SESSION['USER']['userDetails']['0']['user_join_date'];
...
function getuserimage($userid,$imgheight,$imgwidth){
	...
	$userId = $this->userName->userId;
        ...
        $filename=HTTP_SECURE.'/images/secure/user_image/'.$userId.'/'.$result;
                    list($width, $height) = getimagesize($filename);
  
                    $extension = $class->getExtension($result);
                    //echo $extension;
		 if($extension=="jpg" || $extension=="jpeg" ){
			$imageMedium = imagecreatefromjpeg($filename);
			}
			...
			$imageMedium = imagecreatefromgif($filename);
                    ...
                    $image_m =imagecreatetruecolor($mediumWidth, $mediumHeight);
                    imagecopyresampled($image_m, $imageMedium, 0, 0, 0, 0, $mediumWidth, $mediumHeight, $width, $height);
                    $mainPathMedium = "images/secure/user_image/".$this->userName->userId.'/';
                    $mainPathMedium =$mainPathMedium.$this->userName->userId.'_'.$result;
...
function getuserimageSrc($userid,$imgheight,$imgwidth,$size,$title=0,$src=0){
	...
	if($src==0)
	return "<img src='".HTTPS_SECURE."/images/secure/user_image/".$imageName."' alt='".$title."' title='".$title."' height='".$imgheight."' width='".$imgwidth."'>";
	else
	return HTTPS_SECURE."/images/secure/user_image/".$imageName;
...
function checkUserModuleActionPermission($store_apikey,$user_email_id,$mod_id,$action_id=''){
	 ...
	 if($result_user_id==NULL) return false;
...
function displaySettingMenuNew(){
	...
	if($controllerName=='accountsetting'){
		...
		$xyz .='<div class="mainMenu"><a href="http://secure.goo2ostore.com/accountsetting/" title="Account Setting">Account Settings</a></div>';
	}
	if($controllerName=='mypayment'){
		$xyz .='<div class="mainMenuselected">My Payment</div>';
	} else { 
		$xyz .='<div class="mainMenu"><a href="http://secure.goo2ostore.com/mypayment" title="My Payment">My Payment</a></div>';
	}
	if($userAdmin==2){
		if(($controllerName=='general')||($controllerName=='location')||($controllerName=='support')||($controllerName=='shipping')||($controllerName=='policy')||($controllerName=='user-setting')||($controllerName=='verification')){
			$xyz .='<div class="mainMenuselected">Store</div>';
		} else { 
			$xyz .='<div class="mainMenu"><a href="http://goo2ostore.com/settings/general/storedetail" title="Store">Store</a></div>';
		}
		if($actionName=='storedetail'){
			$xyz .='<div class="subMenuselected">General</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/general/storedetail" title="General">General</a></div>';
		}
		if($actionName=='storelocations'){
			$xyz .='<div class="subMenuselected">Store Location</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/location/storelocations" title="Store Location">Store Location</a></div>';
		}
		if(($controllerName=='support')&&($actionName=='index')){
			$xyz .='<div class="subMenuselected">Support</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/support" title="Support">Support</a></div>';
		}
		if($actionName=='manageshipping'){
			$xyz .='<div class="subMenuselected">Shipping Policy</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/shipping/manageshipping" title="Shipping Policy">Shipping Policy</a></div>';
		}
		if($actionName=='terms-policies'){
			$xyz .='<div class="subMenuselected">Returns & Cancellation policy</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/policy/terms-policies" title="Returns & Cancellation policy">Returns & Cancellation policy</a></div>';
		}
		/*if($actionName=='user-listing'){
			$xyz .='<div class="subMenuselected">Users</div>';
		} else { 
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/user-setting/user-listing">Users</a></div>';
		}
		if($actionName==''){
			$xyz .='<div class="subMenuselected">Users</div>';
		} else { 
		...
		if(($controllerName=='verification')&&($actionName=='index')){
			...
			$xyz .='<div class="subMenu"><a href="http://goo2ostore.com/settings/verification" title="Verification">Verification</a></div>';
		}		
	}
	$xyz .= '</div>';
	return $xyz;
	...
	public function getMallUserDetailByApikey($owner_apikey){
        ...
        function catbrandproductdetailfromid($id, $type) {
            $returnArray = array();
            $db = $this->databaseconnection();
            if ($type == 'product') {
                $select = $db->select()->from(array('product'), array('*'))
                        ->where('id=' . $id);
            } else if ($type == 'category') {
                $select = $db->select()->from(array('addcategory'), array('*'))
                        ->where('cat_id=' . $id);
            } else if ($type == 'brand') {
                $select = $db->select()->from(array('brand'), array('*'))
                        ->where('brand_id=' . $id);
            }
            $result = $db->fetchRow($select);
