...
class Xml
{
	...
	{
                $this->XMLtext='';
		$this->array_to_xml_transform($arr);
		return $this->XMLtext;
	}
    public function array_to_xml_transform($array){
        static $Depth;
        foreach($array as $key => $value){
            if(!is_array($value)){
                ...
                if(preg_match("/^[0-9]\$/",$key)) $key = "n$key";
                 $this->XMLtext .= "$Tabs<$key>".htmlspecialchars($value,ENT_QUOTES, 'UTF-8')."</$key>\n";
            } else {
                $Depth += 1;
                unset($Tabs);
                ...
                if(!preg_match("/(-attr)\$/", $key)) {
                    ...
                    $this->array_to_xml_transform($value);
...
$obj = new Xml();
$array = $obj->xml2array($xml_str);
echo "<pre>";
print_r($array);
$xml_str = $obj->array2xml($array);
echo $xml_str;
