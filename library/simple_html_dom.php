...
// dump html dom tree
$dom_str = '';
//$class_array = TemplateConfig :: $CLASS_ARRAY;
//$attr_array = TemplateConfig :: $ATTRIBUTE_ARRAY; 
// helper functions
// -----------------------------------------------------------------------------
...
//Get the tree of class,type,rel,src of a block
function block_dump_html_tree($node,$id)
{
	$class_array = TemplateConfig :: $CLASS_ARRAY;
	$attr_array = TemplateConfig :: $ATTRIBUTE_ARRAY; 
	global $dom_str;//,$class_array,$attr_array;
	//Zend_Registry::get("log")->debug("block_dump_html_tree :: id = ".$id." class array = ".Zend_Debug::dump($class_array));
	if(in_array($node->attr['class'],$class_array) || $node->attr['id'] == $id )
	{
		$dom_str .= '(';
		$dom_str .= str_replace('text','',$node->tag);
		foreach($node->attr as $k=>$v)
		{
			if(in_array($k,$attr_array))
            $dom_str .= "[$k=".$node->$k.']';
		}
		foreach($node->nodes as $c)
        	block_dump_html_tree($c,$id);
		$dom_str .= ')';
	}	
}
...
class simple_html_dom_node {
    ...
    //dump block's tree
	function html_dump($id) {
		global $dom_str;
		$dom_str='';
		block_dump_html_tree($this, $id);
		//Zend_Registry::get("log")->debug("html_dump :: tree = ".$dom_str);
        return($dom_str);
    }
