...
echo "Test WWWWWWWWWW";
            ...
            case '58':      //confirm shipment from seller to buyer
                $this->toMailInfo('SET-INFO', array('to_id' => $data_arr['orderData'][0]['buyer_id'], 'to_mail' => $data_arr['orderData'][0]['buyer_email'], 'to_name' => $data_arr['orderData']['buyer_name']));
                $this->fromMailInfo('SET-INFO', array('from_id' => $data_arr['orderData'][0]['seller_id'], 'from_mail' => $data_arr['orderData'][0]['seller_email'], 'from_name' => $data_arr['orderData'][0]['title']));
                $this->ph = array(
            ...
            case '83':           //payment for an order is declined from goo2o to buyer in superadmin module
                ...
                break;
                  case '88':
                      $this->toMailInfo('SET-INFO', array('to_id' => $data_arr['userID'], 'to_mail' => $data_arr['user_email_address'], 'to_name' => $data_arr['user_full_name']));
                      $this->ph = array(
                          'STORE_OWNER_EMAIL_ID'=>$data_arr['triggerDetail']['user_email_address'],
                          'STORE_NAME'=>$data_arr['triggerDetail']['title'],
                          'STORE_URL'=>$data_arr['triggerDetail']['mall_url'],
                          'STORE_OWNER_CONTACT_NUMBER'=>$data_arr['triggerDetail']['user_mobile'],
                          'BUSINESS_NAME'=>$data_arr['triggerDetail']['business_name'],
                          'BUSINESS_STATUS'=>$data_arr['triggerDetail']['businessStatus'],
                          'STORE_OWNER_NAME '=>$data_arr['triggerDetail']['user_full_name']
                      );
                      break;
            default:
